...
<script>
    ...
    document.addEventListener("DOMContentLoaded", async function () {
        ...
        $('body').on('click', '.btn-pagar', async function(){
            // Inicializar pago
            await activarPrestacionesInicializar('PRESTACION')
        })
    ...
    async function generarTurno(detalle, crearPtx = false){
        ...
        // if(detalle != []){
        if (crearPtx) {
            let pre_trx = await activarPrestacionesInicializar('TURNO');
    ...
    async function activarPrestacionesInicializar(origen = 'CHEQUEO'){
        ...
        if(data.code == 200){
            ...
            if(origen == "PRESTACION"){
                datosPago = {};
                datosPago.detalle = dataCita.reservaPaquete;
                datosPago.inicializar = data.data;
                await agregarItemTurno(idPreTransaccion, "PRESTACION");
            }else{
                await agregarItemTurno(idPreTransaccion, "TURNO");
    ...
    async function agregarItemTurno(idPreTransaccion, origen = "TURNO"){
        // let dataAttr = $('.item-coincidencia-selected').attr("data-rel");
        ...
        payload.paquetesPromocionales = {
            "_id": generateUUIDv4(),
            "secuenciaPaquetePaciente": dataCita.reservaPaquete.secuanciaPaquetePaciente
        }
