            ...
            $.each(detalle.detallesOrden, function(key, value){
                ...
                if(estadosVigentes.includes(value.codigoEstado)){
                    ...
                    prestacionesPorPagar += `<li class="list-group-item bg-white border-0 mb-2 py-0 fs-16 ms-1 p-0 line-height-16 d-flex justify-content-start align-items-center">
                        ${ badge }
                        ${ (detalle.tipoServicio == "ORDEN_MEDICA" && (detalle.nombreServicioNivel1 == "CONSULTA" || value.codigoServicio == 252)) ? value.nombreServicio : value.nombrePrestacion }
                        ${btnTerapia}
                    </li>`
                }
...
return;
            ...
            if($(this).attr('terapia-rel') !== null && $(this).attr('terapia-rel') == "S"){
                esTerapia = true;
                permitePago = generales.permitePago;
        ...
        $('body').on('click', '.btn-continuar-voucher', async function(){
            ...
            let validacion = await validarDatosVoucher();
            if(validacion){
    ...
    async function validarDatosVoucher(){
        return true;
        let val = $('#telefonoV').val().replace(/\D/g, '');
        let msg = "";

        if (val.startsWith('0')) {
            if (val.length !== 10) {
                msg += "Teléfono móvil incorrecto \n";
            }
        }else{
            if (val.length !== 9) {
                msg += "Teléfono móvil incorrecto \n";
        ...
        if($('#numeroIdentificacionV').val() == ""){
            msg += "Debe ingresar una cédula válida \n";
        }

        if($('#nombreCompletoV').val() == ""){
            msg += "Debe ingresar nombres completos \n";
        }
        ...
        if(msg == ""){
            ...
            toastr.error(msg, 'Datos de Voucher incorrectos', {
                timeOut: 8000
            });
            return false;
        }
        ...
        switch(detalle.tipoServicio){
            ...
            case 'ORDENES_APOYO_PENDIENTE':
                ...
                if(detalle.nombreServicioNivel1 == "PROCEDIMIENTOS"){
                    ...
                    textColorServicio = `text-green-dark`;
                    icon_service_name = `{{ request()->getHost() === '127.0.0.1' ? url('/') : secure_url('/') }}/assets/img/svg/laboratorio-ico.svg`;  
                }else if(detalle.tipoServicio == 'TERAPIA_FISICA'){
            ...
            switch(item.tipoServicio){
                case 'ORDEN_MEDICA':
                    if(item.nombreServicioNivel1 == "PROCEDIMIENTOS"){
                        ...
                        if(item.detallesOrden[0].codigoServicio == 252){
                            tipoServicioItem = 'TERAPIA_FISICA'//item.detallesOrden[0].nombreServicio;
                            labelServicio = 'Terapia Física'//item.detallesOrden[0].nombreServicio;
                        }
                    ...
                    $('#pills-Hoy').html(`
                        <div class="row row-flex mb-3 pb-3">
                            <div class="col-10 col-md-6 mx-auto text-center mt-3">
                                ...
                                <button type="button" class="btn fw-normal text-white w-100 fs-40 badge bg-veris px-4 py-3 btn-turno rounded-8">Generar turno</button>
                            </div>
                        </div>
                    `)
                }
